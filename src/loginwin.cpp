    ...
    ui(new Ui::LoginWin)
{
    ...
    ui->loginButton->setStyleSheet(QString("QPushButton{border-image:url(:/pixmap/image/loginButton.png);color: rgb(255, 255, 255);}QPushButton:focus{background-color: rgb(255, 0, 0);}"));
    ui->loginButton->setFocusPolicy(Qt::NoFocus);
    ui->passwd->setEchoMode(QLineEdit::Password);
    ui->loginButton->setShortcut(QKeySequence::InsertParagraphSeparator);

    setFocus();
    ui->passwd->setPlaceholderText(tr("请输入密码:"));
    ui->username->lineEdit()->setPlaceholderText("请输入人人网ID号");

    ServerAddress = "talk.renren.com";
...
void LoginWin::action_setting()
{
    ...
    if(ServerAddress=="talk.renren.com")
        ui->username->lineEdit()->setPlaceholderText("请输入人人网ID号:");
    else
